...
function loadtest_coop() {
    for batch in "100" "1000" "10000" "100000"; do
    for limit in "${limits[@]}" ; do
        wrk $wrk_args "http://localhost:8080/cpu4?limit=$limit&batch=$batch"
        sleep 30
    done
    done
}
...
run_loadtest "blockingIO" "completableFutureIO" "deferredIO1" "deferredIO2" "deferredIO3"
run_loadtest_cpu "cpu0" "cpu1" "cpu2" "cpu3" "cpu0semaphore"
loadtest_coop
